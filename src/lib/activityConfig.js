...
export const CATEGORY_ORDER = [
  "Salah","Health","Islamic Learning","Programming","Learning","Work",
  "Aptitude","English / Interview","Family & Social","Life Skills",
  "Personal","Reflection / Night","Other / Custom",
];
...
export const DEFAULT_DAILY = {
  ...
  "Flexible Learning / Creative Practice / Programming": { time: "08:15", category: "Programming" },
...
export const DEFAULT_WEEKLY = {
  "Programming Technology + English Grammar": { category: "Programming", days: ["Mon"] },
  "Aptitude Practice + Mock Interviews": { category: "Aptitude", days: ["Tue"] },
  "Programming Concepts (New Tech) + Vocabulary": { category: "Programming", days: ["Wed"] },
  "Aptitude Tests + Speaking Practice": { category: "Aptitude", days: ["Thu"] },
  "Programming Projects + Jumu'ah & Islamic Studies": { category: "Programming", days: ["Fri"] },
  "English Mock Interviews + Programming Revision": { category: "English / Interview", days: ["Sat"] },
  ...
  "Sunday Habit Tracking (8:00 AM)": { category: "Reflection / Night", days: ["Sun"] },
  "Weekly Diverse Content (8:15 AM Block)": { category: "Learning", days: ["Sun"] },
  "Sunday Networking (8:30 PM)": { category: "Work", days: ["Sun"] },
};

export const DEFAULT_WEEKLY_ITEMS = Object.entries(DEFAULT_WEEKLY).map(([name, v]) => ({ name, days: v.days || [] }));
...
export function getDailyMeta(name, customMeta = {}) {
  ...
  if (custom) {
    ...
    return { category: custom.category || "Other / Custom", time, timeLabel: formatTimeLabel(time), sortKey: time || "99:99" };
  }
  const def = DEFAULT_DAILY[name];
  if (def) {
    const time = def.time || "";
    return { category: def.category || "Other / Custom", time, timeLabel: formatTimeLabel(time), sortKey: time || "99:99" };
  }
  return { category: "Other / Custom", time: "", timeLabel: "", sortKey: "99:99" };
}

export function getWeeklyMeta(name) {
  const def = DEFAULT_WEEKLY[name];
  return { category: def?.category || "Other / Custom", time: "", timeLabel: "" };
}
